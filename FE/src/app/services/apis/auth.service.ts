...
export class AuthService extends ApiService {
  ...
  forgotPassword(email: string): Observable<any> {
    return this._http.post(
      API_ENDPOINT.auth.base + API_ENDPOINT.auth.forgotPassword,
      { email },
  ...
  verifyOtp(email: string, otp: string): Observable<any> {
    return this._http.post(
      API_ENDPOINT.auth.base + API_ENDPOINT.auth.otp,
      { email, otp },
      { withCredentials: true }
    );
  }
  
  resetPassword(data: { email: string, password: string, re_password: string }): Observable<any> {
    return this.post(
